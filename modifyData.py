...
from createDatab import updateData, showTable
import datetime
...
class modifyForm:
    def __init__(self, master):
        ...
        self.fetchButton = ttk.Button(
            self.buttonFrame, text="Search", command=lambda: self.modifyinDb()
        )
        self.submitButton = ttk.Button(
            self.buttonFrame, text="Submit", command=lambda: self.updateinDb()
        )
        ...
        self.fetchButton.pack(fill="both", expand=True, side=LEFT)
        self.submitButton.pack(fill="both", expand=True, side=RIGHT)
    ...
    def modifyinDb(self):
        ...
        if firstNameValue:
            ...
            if data:
                ...
                self.lastNameEntry.insert(0, data[1])
                self.genderVar.set(data[2])
                self.ageEntry.delete(0, END)
                self.ageEntry.insert(0, data[3])
                self.vaccineVar.set(data[4])
                self.stateVar.set(data[5])
                self.timeVar.set(data[7])
                self.datecalendar.set_date(
                    datetime.datetime.strptime(data[6], "%Y-%m-%d")
                )
            else:
